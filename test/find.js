...
exports['a simple find many query'] = function(t) {
    setup();
    blogs.findMany({}, function(err,result){
        if(err) throw err;
...
exports['a simple find one query'] = function (t) {
    setup();
    blogs.findOne({criteria: { userId: 1 }}, function(err,result){
        if(err) throw err;
...
exports['a simple findById query'] = function (t) {
    setup();
    blogs.findById(1, function(err,result){
        if(err) throw err;
...
exports['a query with sorted results'] = function (t) {
    setup();
    blogs.findMany({ sort: { createdAt: "DESC" }}, function(err,result){
        if(err) throw err;
        t.equal(data.params.length, 0);
...
exports['a query with paging (offset, limit, and sort)'] = function (t) {
    setup();
    blogs.findMany({ offset:100, limit:200, sort: { createdAt: 'ASC' }}, function(err,result){
    if(err) throw err;
    t.equal(data.params.length, 0);
...
exports['a find many with multiple criteria and comparison operator'] = function (t) {
    ...
    blogs.findMany({ criteria: { userId: 216, createdAt: { GT: aWeekAgo }}}, function(err,result){
        if(err) throw err;
...
exports['a find many with between comparison operator'] = function (t) {
    ...
    var aWeekAgo = new Date(Date.now() - 7 * 24 * 3600 * 1000);
    var now = new Date();
    blogs.findMany({ criteria:{ createdAt:{ BETWEEN: [aWeekAgo, now] }}}, function(err,result){
        if(err) throw err;
...
exports['a query for a column with a null value'] = function (t) {
    setup();
    blogs.findMany({ criteria: { text: null }}, function(err,result){
        if(err) throw err;
        t.equal(data.params.length, 0);
